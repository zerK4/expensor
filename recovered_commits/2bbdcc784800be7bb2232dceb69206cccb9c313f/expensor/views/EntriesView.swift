        ...
        NavigationView {
            ZStack(alignment: .top) {
                ...
                .onPreferenceChange(ScrollOffsetPreferenceKey.self) { value in
                    ...
                    // Calculate the difference from the last content offset
                    let diff = value - lastContentOffset
                    
                    // Update scroll direction only for significant changes
                    if abs(diff) > 5 {
                        withAnimation(.easeInOut(duration: 0.3)) {
                            if diff > 0 && value > -50 {
                                // Scrolling down or at top
                                showSearch = true
                            } else if diff < 0 && abs(value) > 10 {
