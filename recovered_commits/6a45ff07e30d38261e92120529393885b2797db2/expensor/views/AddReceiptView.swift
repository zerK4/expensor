    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                Spacer()
                Stepper("", value: $item.quantity, in: 1...100) { increment in
                    if !increment && item.quantity == 1 && canBeRemoved {
                        onValueChange(true)
                    }
                }
                .fixedSize()
            ...
            Text("Total: \(String(format: "%.2f lei", total))")
                .font(.subheadline)
        ...
        .onChange(of: item.unitPrice) { _ in
            item.total = total
        }
    }
}
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Items")) {
                    ForEach(Array(newReceipt.items.enumerated()), id: \.offset) { index, _ in
                        ItemRowView(
                            item: $newReceipt.items[index],
                            onValueChange: { shouldRemove in
                                if shouldRemove {
                                    withAnimation {
                                        if index < newReceipt.items.count && newReceipt.items.count > 1 {
                            ...
                            canBeRemoved: newReceipt.items.count > 1
                        )
                    }
