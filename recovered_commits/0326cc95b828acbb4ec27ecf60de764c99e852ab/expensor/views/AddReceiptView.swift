    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                Spacer()
                Stepper("", value: $item.quantity, in: canBeRemoved ? 1...100 : 1...100) {
                    if item.quantity == 1 && canBeRemoved {
                        onValueChange()
                    }
                }
                .fixedSize()
            ...
            Text("Total: \(String(format: "%.2f lei", item.unitPrice * Double(item.quantity)))")
                .font(.subheadline)
