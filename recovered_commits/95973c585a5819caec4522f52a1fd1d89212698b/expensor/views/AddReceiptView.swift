...
private struct PaymentToggleView: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            // Card Payment Button
            Button(action: {
                paidCard = paidCard == nil ? total : nil
                if paidCard != nil { paidCash = nil }
            }) {
                VStack {
                ...
                .padding(.vertical, 12)
                .background(paidCard != nil ? Color.blue.opacity(0.2) : Color.gray.opacity(0.1))
                .foregroundColor(paidCard != nil ? .blue : .gray)
                .cornerRadius(12)
                .overlay(
                    RoundedRectangle(cornerRadius: 12)
                        .stroke(paidCard != nil ? Color.blue : Color.gray.opacity(0.3), lineWidth: 1)
                )
            }
            
            // Cash Payment Button
            Button(action: {
                paidCash = paidCash == nil ? total : nil
                if paidCash != nil { paidCard = nil }
            }) {
                VStack {
                ...
                .frame(maxWidth: .infinity)
                .padding(.vertical, 12)
                .background(paidCash != nil ? Color.green.opacity(0.2) : Color.gray.opacity(0.1))
                .foregroundColor(paidCash != nil ? .green : .gray)
                .cornerRadius(12)
                .overlay(
                    RoundedRectangle(cornerRadius: 12)
                        .stroke(paidCash != nil ? Color.green : Color.gray.opacity(0.3), lineWidth: 1)
                )
            }
        }
        .padding(.vertical, 4)
